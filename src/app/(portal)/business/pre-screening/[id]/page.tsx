...
export default async function BusinessPreScreening({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <Link href={`/business/application/${params.id}`} className="inline-flex items-center gap-1.5 text-sm font-medium mb-4" style={{ color: '#5B7568' }}>
      ...
      <PreScreeningForm applicationId={params.id} initialAnswers={ps?.answers ?? {}} locked={locked} homeHref={`/business/application/${params.id}`} />
    </div>
  )
}

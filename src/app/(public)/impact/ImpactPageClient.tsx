...
import { IMPACT_STAT_COLORS, type ImpactSettings } from '@/lib/impact'
...
export default function ImpactPageClient({ impact }: { impact: ImpactSettings }) {
  const { lang } = useLang()

  const YEAR_STATS = impact.growth
  const maxCertified = Math.max(1, ...YEAR_STATS.map(y => y.certified))
      ...
      <section className="section-white py-16 border-b border-mint">
        <div className="container-fee">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {impact.heroStats.map((s, i) => ({ ...s, color: IMPACT_STAT_COLORS[i % IMPACT_STAT_COLORS.length] })).map((stat, i) => (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <FadeIn>
              ...
              <div className="grid grid-cols-3 gap-4">
                {[
                  { n: impact.countries, l_en: 'Countries', l_ar: 'دولة' },
                  { n: impact.memberOrgs, l_en: 'Member Orgs', l_ar: 'منظمة عضو' },
                  { n: impact.established, l_en: 'Established', l_ar: 'التأسيس' },
                ].map((s) => (

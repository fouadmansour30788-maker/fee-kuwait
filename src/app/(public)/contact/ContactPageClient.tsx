...
export default function ContactPageClient({ settings }: { settings: SiteSettings }) {
  ...
  const CONTACT_INFO = [
    { icon: Mail, label_en: 'Email', label_ar: 'البريد الإلكتروني', value: settings.contact_email, color: '#52B788' },
    { icon: Phone, label_en: 'Phone', label_ar: 'الهاتف', value: settings.contact_phone, color: '#006994' },
    { icon: User, label_en: 'Contact Person', label_ar: 'الشخص المسؤول', value: settings.contact_person, color: '#40916C' },
    { icon: MapPin, label_en: 'Address', label_ar: 'العنوان', value: lang === 'ar' ? settings.address_ar : settings.address_en, color: '#C8A951' },
    { icon: Clock, label_en: 'Office Hours', label_ar: 'ساعات العمل', value: lang === 'ar' ? settings.hours_ar : settings.hours_en, color: '#40916C' },
  ]
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            ...
            <FadeIn>
                ...
                {CONTACT_INFO.map((item, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-start gap-4">
                      <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: `${item.color}15`, border: `1px solid ${item.color}30` }}>
                        <Icon className="w-5 h-5" style={{ color: item.color }} />
                      </div>
                      <div>
                        <p className="text-xs font-semibold text-gray uppercase tracking-wider mb-0.5">{lang === 'ar' ? item.label_ar : item.label_en}</p>
              ...
              <div className="card p-6">
                <h3 className="font-bold text-forest text-sm mb-4">{lang === 'ar' ? 'أسئلة شائعة' : 'Quick FAQs'}</h3>
                <div className="space-y-3">
                  {[
                    { q_en: 'How long does certification take?', q_ar: 'كم تستغرق عملية الاعتماد؟', a_en: 'Typically 3–6 months depending on the programme.', a_ar: 'عادةً من 3 إلى 6 أشهر حسب البرنامج.' },
                    { q_en: 'Is there a registration fee?', q_ar: 'هل هناك رسوم تسجيل؟', a_en: 'Yes — fees vary by programme and institution type.', a_ar: 'نعم — تتفاوت الرسوم حسب البرنامج ونوع المؤسسة.' },
                    { q_en: 'How often is certification renewed?', q_ar: 'كم مرة يتم تجديد الاعتماد؟', a_en: 'Most certifications are renewed annually.', a_ar: 'تُجدَّد معظم الشهادات سنوياً.' },
                  ].map((faq, i) => (
            ...
            <FadeIn delay={0.15}>
              {sent ? (
                <div className="card p-12 flex flex-col items-center justify-center text-center h-full">
                  ...
                  <h3 className="text-2xl font-bold text-forest mb-3">{lang === 'ar' ? 'تم إرسال رسالتك!' : 'Message Sent!'}</h3>
                  <p className="text-gray text-sm leading-relaxed max-w-xs">
                    {lang === 'ar' ? 'شكراً لتواصلك معنا. سيرد فريقنا خلال يوم عمل واحد.' : 'Thank you for reaching out. Our team will respond within one business day.'}
                  </p>
                </div>
              ) : (
                <div className="card p-8">
                  <h2 className="text-2xl font-bold text-forest mb-6">{lang === 'ar' ? 'أرسل رسالة' : 'Send a Message'}</h2>
                  <form onSubmit={handleSubmit} className="space-y-5">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="label">{lang === 'ar' ? 'الاسم' : 'Name'}</label>
                        <input type="text" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="input" placeholder={lang === 'ar' ? 'اسمك الكامل' : 'Your full name'} />
                      </div>
                      <div>
                    ...
                    <div>
                      ...
                      <textarea required rows={5} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} className="input resize-none" placeholder={lang === 'ar' ? 'اكتب رسالتك هنا...' : 'Write your message here...'} />
                    </div>

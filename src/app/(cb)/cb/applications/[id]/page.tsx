...
export default async function CbApplicationDetail({
  ...
  return (
    <div className="space-y-5">
      ...
      {/* CB assigns the auditor — during pre-audit review, at cb_review (legacy),
          or once approved for audit. Assigning an auditor proceeds to the audit. */}
      {(app.status === 'cb_pre_audit_review' || app.status === 'cb_pre_audit_re_review' || app.status === 'cb_review' || app.status === 'ready_for_auditor') && (
        <CbReviewPanel applicationId={id} auditors={cbAuditors} />
      )}

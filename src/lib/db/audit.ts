...
export async function cbApplications(): Promise<AppRow[]> {
  const supabase = createClient()
  const { data, error } = await supabase
    .from('applications')
    .select('id, programme, status, entity_type, submitted_at, review_deadline, applicant:users!applicant_id(email, name_en)')
...
// The certification body currently assigned to an application (for the operator view).
export async function applicationCb(applicationId: string): Promise<AuditorUser | null> {
  const supabase = createClient()
  const { data } = await supabase.from('applications').select('cb_id').eq('id', applicationId).single()
  if (!data?.cb_id) return null
  const { data: u } = await supabase.from('users').select('id, name_en, email').eq('id', data.cb_id).single()
  return (u ?? null) as AuditorUser | null
}

...
-- Additive & idempotent — safe to run (or re-run) on the existing database.
--
-- Note: a `partners` table already exists from 001_initial_schema.sql with fewer
-- columns, so this migration ALTERs it (adds the CMS columns) rather than
-- recreating it.
...
-- ── Partners (extend the existing table with CMS columns) ─────────────
alter table public.partners add column if not exists initials   text;
alter table public.partners add column if not exists color      text default '#40916C';
alter table public.partners add column if not exists desc_en    text;
alter table public.partners add column if not exists desc_ar    text;
...
-- Staff can read all (incl. inactive) and manage; public read of active rows is
-- already covered by the existing "Anyone can read active partners" policy.
...
-- Seed the Kuwait partners only if the table is currently empty.
...
) as v(name_en, name_ar, type, logo_url, initials, color, desc_en, desc_ar, website, sort_order)
